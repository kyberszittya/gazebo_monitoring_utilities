...
TEST(BasicTestDistanceNarrowphase, TestBox2Box)
{
    ...
    ASSERT_DOUBLE_EQ(1.0, calc_plugin.getMinTargetDistanceNarrowphase());
    const Eigen::Vector3d tr2_1(-0.5, 1.0, 0.0);
    calc_plugin.updateObjectPose(name2, tr2_1, ori2);
    ASSERT_DOUBLE_EQ(0.5, calc_plugin.getMinTargetDistanceNarrowphase());
    const Eigen::Vector3d tr2_2(0.0, 1.0, 0.0);
    calc_plugin.updateObjectPose(name2, tr2_2, ori2);
    ASSERT_DOUBLE_EQ(0.0, calc_plugin.getMinTargetDistanceNarrowphase());
...
TEST(BasicTestDistanceNarrowphase, TestBox2Box1000)
{
    gazebo::DistanceCalculation calc_plugin;
    // Set target
    ...
    calc_plugin.setupCollision();
    auto start = std::chrono::high_resolution_clock::now();
    double distance = calc_plugin.getMinTargetDistanceNarrowphase();
    auto finish = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = finish - start;
...
TEST(BasicTestDistanceNarrowphase, TestBox2BoxStatic500)
{
    gazebo::DistanceCalculation calc_plugin;
    // Set target
    std::string name1("box_target");
...
TEST(BasicTestDistanceNarrowphase, TestBox2Sphere)
{
    gazebo::DistanceCalculation calc_plugin;
    // This is the dynamic object
...
TEST(BasicTestDistanceNarrowphase, TestBox2Cylinder)
{
    gazebo::DistanceCalculation calc_plugin;
    // This is the dynamic object
...
TEST(BasicTestDistanceNarrowphase, TestBasicUpdate)
{
    gazebo::DistanceCalculation calc_plugin;
    // This is the dynamic object
    std::string name1("box_target");
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        ASSERT_NEAR(1.0+i*UPDATE_DX, calc_plugin.getMinTargetDistanceNarrowphase(), 1e-6);
    }
}
